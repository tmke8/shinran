    ...
    for (key, result) in scope {
        match result {
            ExtensionOutput::Single(value) => {
                let name = format!("SHINRAN_{}", key.to_uppercase());
                output.insert(name, value.clone());
            }
            ExtensionOutput::Multiple(values) => {
                for (sub_key, sub_value) in values {
                    let name = format!("SHINRAN_{}_{}", key.to_uppercase(), sub_key.to_uppercase());
...
mod tests {
    ...
    fn test_convert_to_env_variables() {
        ...
        assert_eq!(output.get("SHINRAN_FORM1_NAME").unwrap(), "John");
        assert_eq!(output.get("SHINRAN_FORM1_LASTNAME").unwrap(), "Snow");
        assert_eq!(output.get("SHINRAN_VAR1").unwrap(), "test");
    }
}
